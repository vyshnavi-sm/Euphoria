...
const applyProductOffer = async (req, res) => {
  try {
    ...
    if (isNaN(start) || isNaN(end) || start >= end) {
      return res
        .status(STATUS_CODE.BAD_REQUEST)
        .json({ success: false, message: "Invalid or mismatched date range" });
    }
    ...
    product.salePrice =
      Math.round(product.regularPrice * (1 - discount / 100) * 100) / 100;
    await product.save();
    ...
    res
      ...
      .json({ success: false, message: "Failed to apply offer" });
  }
};
...
const removeProductOffer = async (req, res) => {
  try {
    ...
    res
      .status(STATUS_CODE.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: "Failed to remove offer" });
  }
};
...
const getAllActiveOffers = async (req, res) => {
  try {
    const now = new Date();
    const activeOffers = await CategoryOffer.find({ isActive: true }).populate(
      "category",
      "name"
    );
    ...
    for (let product of products) {
      try {
        ...
        const discountAmount =
          product.regularPrice * (productOfferDiscount / 100);
        product.salePrice =
          productOfferDiscount > 0
            ? Math.max(
                0,
                Math.round((product.regularPrice - discountAmount) * 100) / 100
              )
